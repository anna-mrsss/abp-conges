...
export const api = {
  ...
  getBlockingPeriods: () => fetch("/api/blocking-periods").then(handle),
  addBlockingPeriod: (payload) =>
    fetch("/api/blocking-periods", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    }).then(handle),
  deleteBlockingPeriod: (id) => fetch(`/api/blocking-periods/${id}`, { method: "DELETE" }).then(handle),
  ...
  deleteEmployee: (email) =>
    fetch(`/api/employees/${encodeURIComponent(email)}`, { method: "DELETE" }).then(handle),
};
