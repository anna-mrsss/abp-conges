...
export function isFrenchHoliday(dateStr) {
  const year = toDate(dateStr).getFullYear();
  return frenchHolidays(year).includes(dateStr);
}
export function isNonWorkingDay(dateStr) {
  return isWeekend(dateStr) || isFrenchHoliday(dateStr);
}
// Cycle annuel de congés ABP : du 1er mai au 30 avril de l'année suivante.
export function currentLeavePeriod(today = new Date()) {
  const y = today.getFullYear();
  const m = today.getMonth() + 1; // 1 à 12
  ...
  return {
    ...
    message: `La totalité des congés ${startYear}-${endYear} doit être prise avant le 1er mai ${endYear}.`,
  };
}
