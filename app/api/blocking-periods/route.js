...
// Public (visible sur la page d'accueil, avant authentification).
export async function GET() {
  await ensureSchema();
  const result = await sql`SELECT * FROM blocking_periods ORDER BY date_debut ASC`;
  return NextResponse.json({ blockingPeriods: result.rows.map(rowToBlocking) });
}
...
export async function POST(request) {
  ...
  const id = uid();
  await sql`
    INSERT INTO blocking_periods (id, libelle, date_debut, date_fin)
